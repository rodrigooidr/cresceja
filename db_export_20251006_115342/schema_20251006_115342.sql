...
--
-- Name: ai_usage_sum(uuid, text); Type: FUNCTION; Schema: public; Owner: cresceja
--

CREATE FUNCTION public.ai_usage_sum(p_org_id uuid, p_category text) RETURNS numeric
    LANGUAGE plpgsql
    AS $_$
DECLARE
  v_sum numeric := 0;
  v_has_category boolean;
  v_has_qty_num  boolean;
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='ai_usage' AND column_name='category'
  ) INTO v_has_category;

  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='ai_usage' AND column_name='qty_num'
  ) INTO v_has_qty_num;

  IF v_has_category THEN
    IF v_has_qty_num THEN
      EXECUTE $q$
        SELECT COALESCE(SUM(qty_num),0)
          FROM public.ai_usage
         WHERE org_id = $1
           AND ($2 IS NULL OR category = $2)
      $q$ INTO v_sum USING p_org_id, p_category;
    ELSE
      EXECUTE $q$
        SELECT COALESCE(SUM(NULLIF(qty,'')::numeric),0)
          FROM public.ai_usage
         WHERE org_id = $1
           AND qty ~ '^[0-9]+(\.[0-9]+)?$'
           AND ($2 IS NULL OR category = $2)
    ...
    END IF;
  ELSE
    -- Sem coluna category: soma tudo da org
    IF v_has_qty_num THEN
      EXECUTE $q$
        SELECT COALESCE(SUM(qty_num),0)
          FROM public.ai_usage
         WHERE org_id = $1
      $q$ INTO v_sum USING p_org_id;
    ELSE
      EXECUTE $q$
        SELECT COALESCE(SUM(NULLIF(qty,'')::numeric),0)
          FROM public.ai_usage
...
ALTER FUNCTION public.ai_usage_sum(p_org_id uuid, p_category text) OWNER TO cresceja;
...
--
-- Name: ensure_column(text, text, text, text, text); Type: FUNCTION; Schema: public; Owner: cresceja
--

CREATE FUNCTION public.ensure_column(p_schema text, p_table text, p_column text, p_type text, p_default text DEFAULT NULL::text) RETURNS void
    LANGUAGE plpgsql
    AS $$
...
BEGIN
  ...
  IF NOT v_exists THEN
    EXECUTE format('ALTER TABLE %I.%I ADD COLUMN %I %s', p_schema, p_table, p_column, p_type);
    IF p_default IS NOT NULL THEN
      EXECUTE format('ALTER TABLE %I.%I ALTER COLUMN %I SET DEFAULT %s', p_schema, p_table, p_column, p_default);
    END IF;
    RAISE NOTICE '[ensure_column] %.%.% criada', p_schema, p_table, p_column;
  END IF;
END$$;


ALTER FUNCTION public.ensure_column(p_schema text, p_table text, p_column text, p_type text, p_default text) OWNER TO cresceja;

--
-- Name: ensure_index(text, text, text, text); Type: FUNCTION; Schema: public; Owner: cresceja
--

CREATE FUNCTION public.ensure_index(p_schema text, p_table text, p_column text, p_index_name text) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE v_table_exists boolean; v_col_exists boolean; v_idx_exists boolean;
BEGIN
  SELECT EXISTS (SELECT 1 FROM information_schema.tables
                 WHERE table_schema=p_schema AND table_name=p_table)
    INTO v_table_exists;
  IF NOT v_table_exists THEN RETURN; END IF;

  SELECT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_schema=p_schema AND table_name=p_table AND column_name=p_column)
    INTO v_col_exists;
  IF NOT v_col_exists THEN RETURN; END IF;

  SELECT EXISTS (SELECT 1 FROM pg_class ic
                 JOIN pg_namespace inn ON inn.oid=ic.relnamespace
                 WHERE ic.relkind='i' AND ic.relname=p_index_name AND inn.nspname=p_schema)
    INTO v_idx_exists;
  IF v_idx_exists THEN RETURN; END IF;

  EXECUTE format('CREATE INDEX %I ON %I.%I (%I)', p_index_name, p_schema, p_table, p_column);
END$$;


ALTER FUNCTION public.ensure_index(p_schema text, p_table text, p_column text, p_index_name text) OWNER TO cresceja;

--
-- Name: ensure_uuid_fk(text, text, text, text, text, text, text); Type: FUNCTION; Schema: public; Owner: cresceja
--

CREATE FUNCTION public.ensure_uuid_fk(p_schema text, p_table text, p_column text, p_ref_schema text, p_ref_table text, p_ref_column text DEFAULT 'id'::text, p_on_delete text DEFAULT 'SET NULL'::text) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
  v_tbl_oid           oid;
  ...
  v_idx_name          text := format('idx_%s_%s', p_table, p_column);
  v_fk_name           text := format('fk_%s_%s__%s', p_table, p_column, p_ref_table);
...
BEGIN
  ...
  IF v_tbl_oid IS NULL THEN
    RAISE NOTICE '[ensure_uuid_fk] tabela %.% não existe — pulando %', p_schema, p_table, p_column;
    RETURN;
  END IF;
  ...
  -- Se não existir, cria como UUID NULL
  IF NOT v_col_exists THEN
    EXECUTE format('ALTER TABLE %I.%I ADD COLUMN %I uuid', p_schema, p_table, p_column);
    RAISE NOTICE '[ensure_uuid_fk] coluna %.%.% criada (uuid)', p_schema, p_table, p_column;
  ELSE
    -- Se existir, checar tipo (informativo apenas)
    SELECT data_type
      INTO v_col_type
    FROM information_schema.columns
    WHERE table_schema = p_schema
    ...
    IF v_col_type IS DISTINCT FROM 'uuid' THEN
      RAISE NOTICE '[ensure_uuid_fk] coluna %.%.% já existe com tipo %, não alterado.', p_schema, p_table, p_column, v_col_type;
    END IF;
  END IF;

  -- Índice (se ainda não existe)
  IF NOT EXISTS (
    SELECT 1 FROM pg_class ic
    JOIN pg_namespace inn ON inn.oid = ic.relnamespace
    WHERE ic.relkind = 'i'
      AND ic.relname = v_idx_name
      AND inn.nspname = p_schema
  ) THEN
    EXECUTE format('CREATE INDEX %I ON %I.%I (%I)', v_idx_name, p_schema, p_table, p_column);
    RAISE NOTICE '[ensure_uuid_fk] índice % criado em %.%.%', v_idx_name, p_schema, p_table, p_column;
  END IF;

  -- Existe tabela de referência?
  SELECT EXISTS (
    SELECT 1
  ...
  -- FK (se ref existir e FK ainda não existir)
  IF v_ref_exists THEN
    ...
    IF NOT v_constraint_exists THEN
      EXECUTE format(
        ...
        p_schema, p_table, v_fk_name, p_column, p_ref_schema, p_ref_table, p_ref_column, p_on_delete
      );
      RAISE NOTICE '[ensure_uuid_fk] FK % adicionada (%.%.% -> %.%.%)',
        v_fk_name, p_schema, p_table, p_column, p_ref_schema, p_ref_table, p_ref_column;
    END IF;
  ELSE
    RAISE NOTICE '[ensure_uuid_fk] tabela de referência %.% não existe — FK ignorada.', p_ref_schema, p_ref_table;
  END IF;
END;
$$;


ALTER FUNCTION public.ensure_uuid_fk(p_schema text, p_table text, p_column text, p_ref_schema text, p_ref_table text, p_ref_column text, p_on_delete text) OWNER TO cresceja;

--
-- Name: get_ai_credits_status(uuid); Type: FUNCTION; Schema: public; Owner: cresceja
--

CREATE FUNCTION public.get_ai_credits_status(p_org_id uuid) RETURNS jsonb
    LANGUAGE sql
    AS $$
  SELECT payload
  FROM public.v_ai_credits_status
  WHERE org_id = p_org_id;
$$;


ALTER FUNCTION public.get_ai_credits_status(p_org_id uuid) OWNER TO cresceja;

--
-- Name: get_integrations_status(uuid); Type: FUNCTION; Schema: public; Owner: cresceja
--

CREATE FUNCTION public.get_integrations_status(p_org_id uuid) RETURNS jsonb
    LANGUAGE sql
    AS $$
  SELECT payload
  FROM public.v_org_integrations_status
  WHERE org_id = p_org_id;
$$;


ALTER FUNCTION public.get_integrations_status(p_org_id uuid) OWNER TO cresceja;
...
DECLARE
  ...
  plan_id uuid := NULL;
  col_list text := 'id,name';
  val_list text := quote_literal(p_id)||','||quote_literal(p_name);
BEGIN
  IF has_slug AND p_slug IS NOT NULL THEN
    col_list := col_list||',slug';
    val_list := val_list||','||quote_literal(p_slug);
  END IF;
  IF has_status AND p_status IS NOT NULL THEN
    col_list := col_list||',status';
    val_list := val_list||','||quote_literal(p_status);
  END IF;
  IF has_plan_id AND p_plan_code IS NOT NULL AND public.col_exists('plans','code') THEN
    ...
    IF plan_id IS NOT NULL THEN
      col_list := col_list||',plan_id';
      val_list := val_list||','||quote_literal(plan_id::text);
    END IF;
  END IF;
...
--
-- Name: plans_sync_label_name(); Type: FUNCTION; Schema: public; Owner: cresceja
--

CREATE FUNCTION public.plans_sync_label_name() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  -- Se só name veio, preenche label
  IF NEW.label IS NULL AND NEW.name IS NOT NULL THEN
    NEW.label := NEW.name;
  END IF;

  -- Se só label veio, preenche name
  IF NEW.name IS NULL AND NEW.label IS NOT NULL THEN
    NEW.name := NEW.label;
  END IF;
...
ALTER FUNCTION public.plans_sync_label_name() OWNER TO cresceja;

--
-- Name: safe_create_index(text, text, text, text); Type: FUNCTION; Schema: public; Owner: cresceja
--

CREATE FUNCTION public.safe_create_index(p_index_name text, p_schema_name text, p_table_name text, p_column_name text) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
  v_table_oid oid;
...
BEGIN
  ...
  WHERE n.nspname = p_schema_name
    ...
    AND c.relkind = 'r'; -- tabela

  IF v_table_oid IS NULL THEN
    RAISE NOTICE '[safe_create_index] tabela %.% não existe — pulando índice %', p_schema_name, p_table_name, p_index_name;
    RETURN;
  END IF;
  ...
  IF NOT v_col_exists THEN
    RAISE NOTICE '[safe_create_index] coluna %.%.% não existe — pulando índice %', p_schema_name, p_table_name, p_column_name, p_index_name;
    RETURN;
  END IF;
  ...
  SELECT EXISTS (
    ...
    WHERE ic.relkind = 'i'
      ...
      AND inn.nspname = p_schema_name
  ) INTO v_idx_exists;

  IF v_idx_exists THEN
    RAISE NOTICE '[safe_create_index] índice % já existe — ok', p_index_name;
    RETURN;
  END IF;

  -- cria índice
  EXECUTE format('CREATE INDEX %I ON %I.%I (%I)', p_index_name, p_schema_name, p_table_name, p_column_name);
  RAISE NOTICE '[safe_create_index] índice % criado em %.%.%', p_index_name, p_schema_name, p_table_name, p_column_name;
END;
$$;


ALTER FUNCTION public.safe_create_index(p_index_name text, p_schema_name text, p_table_name text, p_column_name text) OWNER TO cresceja;
...
CREATE TABLE public.message_templates (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    org_id uuid,
    channel_scope text[] DEFAULT ARRAY[]::text[],
    language text,
    body text,
...
--
-- Name: opportunities_view; Type: VIEW; Schema: public; Owner: cresceja
--
...
ALTER VIEW public.opportunities_view OWNER TO cresceja;
...
--
-- Name: org_credits_usage; Type: TABLE; Schema: public; Owner: cresceja
--

CREATE TABLE public.org_credits_usage (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    org_id uuid NOT NULL,
    meter text NOT NULL,
...
ALTER TABLE public.org_credits_usage OWNER TO cresceja;
...
--
-- Name: org_integrations_clean; Type: VIEW; Schema: public; Owner: cresceja
--

CREATE VIEW public.org_integrations_clean AS
 SELECT id,
    org_id,
    provider,
    status,
...
ALTER VIEW public.org_integrations_clean OWNER TO cresceja;

--
-- Name: org_plan_events; Type: TABLE; Schema: public; Owner: cresceja
--

CREATE TABLE public.org_plan_events (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    org_id uuid NOT NULL,
...
ALTER TABLE public.org_plan_events OWNER TO cresceja;
...
--
-- Name: org_settings_clean; Type: VIEW; Schema: public; Owner: cresceja
--

CREATE VIEW public.org_settings_clean AS
 SELECT id,
    org_id,
    ai_enabled,
    ai_handoff_keywords,
    ai_max_turns_before_handoff,
    alert_sound,
    alert_volume,
    allow_baileys,
    business_hours,
    templates_enabled_channels,
    whatsapp_active_mode,
    created_at,
    updated_at
   FROM public.org_settings;


ALTER VIEW public.org_settings_clean OWNER TO cresceja;
...
CREATE TABLE public.plan_credits (
    ...
    plan_id uuid DEFAULT public.gen_random_uuid(),
    updated_at timestamp with time zone DEFAULT now(),
    ai_attendance_monthly_num numeric,
    ai_content_monthly_num numeric
);
...
--
-- Name: quick_replies_clean; Type: VIEW; Schema: public; Owner: cresceja
--

CREATE VIEW public.quick_replies_clean AS
 SELECT id,
...
ALTER VIEW public.quick_replies_clean OWNER TO cresceja;
...
--
-- Name: v_ai_credits_status; Type: VIEW; Schema: public; Owner: cresceja
--

CREATE VIEW public.v_ai_credits_status AS
 SELECT id AS org_id,
    jsonb_build_object('support', jsonb_build_object('used', public.ai_usage_sum(id, 'support'::text), 'limit', COALESCE(( SELECT oc.delta_num
           FROM public.org_credits oc
          WHERE ((oc.org_id = o.id) AND (oc.feature_code = 'support'::text))
          ORDER BY oc.created_at DESC
         LIMIT 1), ( SELECT pc.ai_attendance_monthly_num
           FROM (public.plan_credits pc
             JOIN public.organizations oo ON ((oo.plan_id = pc.plan_id)))
          WHERE (oo.id = o.id)
          ORDER BY pc.created_at DESC
         LIMIT 1)), 'period', 'monthly'), 'content', jsonb_build_object('used', public.ai_usage_sum(id, 'content'::text), 'limit', COALESCE(( SELECT oc.delta_num
           FROM public.org_credits oc
          WHERE ((oc.org_id = o.id) AND (oc.feature_code = 'content'::text))
          ORDER BY oc.created_at DESC
         LIMIT 1), ( SELECT pc.ai_content_monthly_num
           FROM (public.plan_credits pc
             JOIN public.organizations oo ON ((oo.plan_id = pc.plan_id)))
          WHERE (oo.id = o.id)
          ORDER BY pc.created_at DESC
         LIMIT 1)), 'period', 'monthly')) AS payload
   FROM public.organizations o;


ALTER VIEW public.v_ai_credits_status OWNER TO cresceja;
...
CREATE TABLE public.whatsapp_channels (
    ...
    provider text DEFAULT 'whatsapp_session'::text,
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    ...
    settings jsonb,
    last_error text,
    CONSTRAINT chk_whatsapp_channels_status CHECK (((status IS NULL) OR (status = ANY (ARRAY['connected'::text, 'disconnected'::text, 'error'::text, 'unknown'::text]))))
);
...
--
-- Name: v_org_integrations_status; Type: VIEW; Schema: public; Owner: cresceja
--

CREATE VIEW public.v_org_integrations_status AS
 SELECT id AS org_id,
    jsonb_build_object('whatsapp', COALESCE(( SELECT jsonb_agg(jsonb_build_object('channel_id', w.id, 'status', COALESCE(w.status, 'unknown'::text), 'phone', w.phone_e164, 'provider', COALESCE(w.provider, 'whatsapp_session'::text), 'connected_at', w.connected_at, 'disconnected_at', w.disconnected_at) ORDER BY w.created_at) AS jsonb_agg
           FROM public.whatsapp_channels w
          WHERE (w.org_id = o.id)), '[]'::jsonb), 'google_calendar', COALESCE(( SELECT jsonb_agg(jsonb_build_object('account_id', g.id, 'google_user_id', g.google_user_id, 'status', 'connected') ORDER BY g.id) AS jsonb_agg
           FROM public.google_calendar_accounts g
          WHERE (g.org_id = o.id)), '[]'::jsonb), 'other', COALESCE(( SELECT jsonb_agg(jsonb_build_object('integration_id', i.id, 'provider', i.provider, 'status', i.status) ORDER BY i.created_at) AS jsonb_agg
           FROM ( SELECT org_integrations.id,
                    org_integrations.provider,
                    org_integrations.status,
                    org_integrations.created_at
                   FROM public.org_integrations) i), '[]'::jsonb)) AS payload
   FROM public.organizations o;


ALTER VIEW public.v_org_integrations_status OWNER TO cresceja;

--
-- Name: v_user_memberships; Type: VIEW; Schema: public; Owner: cresceja
--

CREATE VIEW public.v_user_memberships AS
 SELECT ou.user_id,
    ou.org_id,
    COALESCE(ou.role, 'OrgViewer'::text) AS role_source,
    'org_users'::text AS source_table
   FROM public.org_users ou
UNION
 SELECT uo.user_id,
    uo.org_id,
    COALESCE(uo.role, 'OrgViewer'::text) AS role_source,
    'user_orgs'::text AS source_table
   FROM public.user_orgs uo;


ALTER VIEW public.v_user_memberships OWNER TO cresceja;

--
-- Name: v_orgs_me; Type: VIEW; Schema: public; Owner: cresceja
--

CREATE VIEW public.v_orgs_me AS
 SELECT u.id AS user_id,
    ...
    o.status,
    o.plan_id,
    array_agg(DISTINCT v.role_source) AS roles
   FROM ((public.users u
     JOIN public.v_user_memberships v ON ((v.user_id = u.id)))
     JOIN public.organizations o ON ((o.id = v.org_id)))
  GROUP BY u.id, o.id, o.name, o.slug, o.status, o.plan_id;


ALTER VIEW public.v_orgs_me OWNER TO cresceja;

--
-- Name: opportunities_view created_at; Type: DEFAULT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.opportunities_view ALTER COLUMN created_at SET DEFAULT now();


--
-- Name: opportunities_view updated_at; Type: DEFAULT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.opportunities_view ALTER COLUMN updated_at SET DEFAULT now();


--
-- Name: org_integrations_clean created_at; Type: DEFAULT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.org_integrations_clean ALTER COLUMN created_at SET DEFAULT now();


--
-- Name: org_integrations_clean updated_at; Type: DEFAULT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.org_integrations_clean ALTER COLUMN updated_at SET DEFAULT now();


--
-- Name: org_settings_clean created_at; Type: DEFAULT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.org_settings_clean ALTER COLUMN created_at SET DEFAULT now();


--
-- Name: org_settings_clean updated_at; Type: DEFAULT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.org_settings_clean ALTER COLUMN updated_at SET DEFAULT now();


--
-- Name: quick_replies_clean created_at; Type: DEFAULT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.quick_replies_clean ALTER COLUMN created_at SET DEFAULT now();


--
-- Name: quick_replies_clean updated_at; Type: DEFAULT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.quick_replies_clean ALTER COLUMN updated_at SET DEFAULT now();
...
--
-- Name: org_credits_usage org_credits_usage_pkey; Type: CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.org_credits_usage
    ADD CONSTRAINT org_credits_usage_pkey PRIMARY KEY (id);
...
--
-- Name: org_plan_events org_plan_events_pkey; Type: CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.org_plan_events
    ADD CONSTRAINT org_plan_events_pkey PRIMARY KEY (id);
...
--
-- Name: contact_identities ux_contact_identities_org_channel_identity; Type: CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.contact_identities
    ADD CONSTRAINT ux_contact_identities_org_channel_identity UNIQUE (org_id, channel, identity);


--
-- Name: contact_identities ux_contact_identity_per_org_channel; Type: CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.contact_identities
    ADD CONSTRAINT ux_contact_identity_per_org_channel UNIQUE (org_id, channel, identity);


--
-- Name: contacts ux_contacts_phone_per_org; Type: CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.contacts
    ADD CONSTRAINT ux_contacts_phone_per_org UNIQUE (org_id, phone_e164);
...
--
-- Name: idx_ai_credit_usage_org_id; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_ai_credit_usage_org_id ON public.ai_credit_usage USING btree (org_id);
...
--
-- Name: idx_ai_meters_org_id; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_ai_meters_org_id ON public.ai_meters USING btree (org_id);
...
CREATE INDEX idx_calendar_events_org_id ON public.calendar_events USING btree (org_id);


--
-- Name: idx_calendars_org_id; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_calendars_org_id ON public.calendars USING btree (org_id);
...
--
-- Name: idx_contact_identities_lookup; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_contact_identities_lookup ON public.contact_identities USING btree (org_id, channel, identity);
...
--
-- Name: idx_contact_tags_org; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_contact_tags_org ON public.contact_tags USING btree (org_id);
...
--
-- Name: idx_contacts_org_name; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_contacts_org_name ON public.contacts USING btree (org_id, name);


--
-- Name: idx_content_campaigns_org_id; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_content_campaigns_org_id ON public.content_campaigns USING btree (org_id);


--
-- Name: idx_conversations_org_channel_last; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_conversations_org_channel_last ON public.conversations USING btree (org_id, COALESCE(channel, ''::text), last_message_at DESC);
...
--
-- Name: idx_conversations_org_status_last; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_conversations_org_status_last ON public.conversations USING btree (org_id, COALESCE(status, 'open'::text), last_message_at DESC);


--
-- Name: idx_conversations_org_tags_gin; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_conversations_org_tags_gin ON public.conversations USING gin (tags);


--
-- Name: idx_crm_opportunities_org_id; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_crm_opportunities_org_id ON public.crm_opportunities USING btree (org_id);
...
--
-- Name: idx_message_status_events_org_id; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_message_status_events_org_id ON public.message_status_events USING btree (org_id);
...
--
-- Name: idx_messages_created_at; Type: INDEX; Schema: public; Owner: cresceja
--
...
--
-- Name: idx_org_credits_org_id; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_org_credits_org_id ON public.org_credits USING btree (org_id);
...
--
-- Name: idx_organizations_order; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_organizations_order ON public.organizations USING btree (COALESCE(nome_fantasia, name, slug));


--
-- Name: idx_organizations_slug; Type: INDEX; Schema: public; Owner: cresceja
--
...
--
-- Name: idx_social_posts_org_id; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_social_posts_org_id ON public.social_posts USING btree (org_id);
...
--
-- Name: idx_users_roles_gin; Type: INDEX; Schema: public; Owner: cresceja
--
...
--
-- Name: idx_whatsapp_channels_phone; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_whatsapp_channels_phone ON public.whatsapp_channels USING btree (phone_e164);


--
-- Name: idx_whatsapp_channels_status; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE INDEX idx_whatsapp_channels_status ON public.whatsapp_channels USING btree (status);
...
--
-- Name: ux_contact_identities_identity; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE UNIQUE INDEX ux_contact_identities_identity ON public.contact_identities USING btree (org_id, channel, account_id, identity);
...
--
-- Name: ux_msgtpl_org_name; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE UNIQUE INDEX ux_msgtpl_org_name ON public.message_templates USING btree (org_id, lower(name));
...
--
-- Name: ux_plans_id_legacy_text; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE UNIQUE INDEX ux_plans_id_legacy_text ON public.plans USING btree (id_legacy_text);
...
--
-- Name: ux_tags_org_name; Type: INDEX; Schema: public; Owner: cresceja
--

CREATE UNIQUE INDEX ux_tags_org_name ON public.tags USING btree (org_id, lower(name));
...
--
-- Name: plans trg_plans_sync_label_name; Type: TRIGGER; Schema: public; Owner: cresceja
--

CREATE TRIGGER trg_plans_sync_label_name BEFORE INSERT OR UPDATE ON public.plans FOR EACH ROW EXECUTE FUNCTION public.plans_sync_label_name();


--
-- Name: whatsapp_channels trg_whatsapp_channels_touch_updated_at; Type: TRIGGER; Schema: public; Owner: cresceja
--

CREATE TRIGGER trg_whatsapp_channels_touch_updated_at BEFORE UPDATE ON public.whatsapp_channels FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();
...
ALTER TABLE ONLY public.ai_credit_usage
    ADD CONSTRAINT fk_ai_credit_usage_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: ai_meters fk_ai_meters_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.ai_meters
    ADD CONSTRAINT fk_ai_meters_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;


--
-- Name: ai_usage fk_ai_usage_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.ai_usage
    ADD CONSTRAINT fk_ai_usage_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: calendar_events fk_calendar_events_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.calendar_events
    ADD CONSTRAINT fk_calendar_events_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;


--
-- Name: calendars fk_calendars_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.calendars
    ADD CONSTRAINT fk_calendars_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: contact_identities fk_contact_identities_contact; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.contact_identities
    ADD CONSTRAINT fk_contact_identities_contact FOREIGN KEY (contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
--
-- Name: contact_tags fk_contact_tags_contact; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.contact_tags
    ADD CONSTRAINT fk_contact_tags_contact FOREIGN KEY (contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
--
-- Name: contact_tags fk_contact_tags_tag; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.contact_tags
    ADD CONSTRAINT fk_contact_tags_tag FOREIGN KEY (tag_id) REFERENCES public.tags(id) ON DELETE CASCADE;
...
--
-- Name: contacts fk_contacts_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.contacts
    ADD CONSTRAINT fk_contacts_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: content_campaigns fk_content_campaigns_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.content_campaigns
    ADD CONSTRAINT fk_content_campaigns_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;


--
-- Name: conversations fk_conversations_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.conversations
    ADD CONSTRAINT fk_conversations_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: google_calendar_accounts fk_google_calendar_accounts_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.google_calendar_accounts
    ADD CONSTRAINT fk_google_calendar_accounts_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: leads fk_leads_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.leads
    ADD CONSTRAINT fk_leads_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: message_status_events fk_message_status_events_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.message_status_events
    ADD CONSTRAINT fk_message_status_events_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;


--
-- Name: message_templates fk_message_templates_org; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.message_templates
    ADD CONSTRAINT fk_message_templates_org FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
...
--
-- Name: messages fk_messages_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.messages
    ADD CONSTRAINT fk_messages_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: opportunities fk_opportunities_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.opportunities
    ADD CONSTRAINT fk_opportunities_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: org_credits fk_org_credits_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.org_credits
    ADD CONSTRAINT fk_org_credits_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: org_integration_logs fk_org_integration_logs_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.org_integration_logs
    ADD CONSTRAINT fk_org_integration_logs_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: org_integrations fk_org_integrations_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.org_integrations
    ADD CONSTRAINT fk_org_integrations_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: org_users fk_org_users_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.org_users
    ADD CONSTRAINT fk_org_users_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: org_users fk_org_users_user_id__users; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.org_users
    ADD CONSTRAINT fk_org_users_user_id__users FOREIGN KEY (user_id) REFERENCES public.users(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: social_posts fk_social_posts_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.social_posts
    ADD CONSTRAINT fk_social_posts_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: telemetry_events fk_telemetry_events_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.telemetry_events
    ADD CONSTRAINT fk_telemetry_events_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: user_orgs fk_user_orgs_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.user_orgs
    ADD CONSTRAINT fk_user_orgs_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: user_orgs fk_user_orgs_user_id__users; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.user_orgs
    ADD CONSTRAINT fk_user_orgs_user_id__users FOREIGN KEY (user_id) REFERENCES public.users(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: users fk_users_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.users
    ADD CONSTRAINT fk_users_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE SET NULL;
...
--
-- Name: whatsapp_channels fk_whatsapp_channels_org_id__organizations; Type: FK CONSTRAINT; Schema: public; Owner: cresceja
--

ALTER TABLE ONLY public.whatsapp_channels
    ADD CONSTRAINT fk_whatsapp_channels_org_id__organizations FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
GRANT CREATE ON SCHEMA public TO cresceja;
