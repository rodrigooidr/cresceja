...
function ConversationItem({ c, onOpen }) {
  ...
  return (
    <button
      onClick={() => onOpen(c)}
      className="w-full px-3 py-2 hover:bg-gray-100 flex gap-3 border-b"
    >
      <img
        src={contact.photo_url || "https://placehold.co/40"}
        alt="avatar"
        className="w-10 h-10 rounded-full"
      />
      <div className="text-left">
        <div className="font-medium flex items-center gap-2">
          <span title={c?.channel} className="text-lg">
            {icon}
          </span>
          {contact.name || contact.phone_e164 || "Contato"}
        </div>
        <div className="text-xs text-gray-500">
          {c?.channel || "canal"} · {c?.status || "status"}
        </div>
      </div>
    </button>
  );
...
export default function InboxPage() {
  ...
  // Socket: novas mensagens + updates (ex.: transcrição pronta)
  useEffect(() => {
    const s = makeSocket();
    ...
    s.on("message:new", (payload) => {
      const convId =
        ...
        payload?.conversationIdOverride;
    ...
    s.on("message:updated", ({ conversationId, message }) => {
      if (!sel?.id || String(sel.id) !== String(conversationId)) return;
  ...
  const send = async () => {
    ...
    try {
      const res = await inboxApi.post(
        `/conversations/${sel.id}/messages`,
      ...
      if (!created) {
        console.warn(
          "Resposta inesperada do servidor ao enviar mensagem:",
          res?.data
        );
        return;
      }
        ...
        {sel ? (
          <div>
            <div className="flex items-center gap-3 mb-3">
              <img
                src={sel?.contact?.photo_url || "https://placehold.co/56"}
                alt="avatar"
                className="w-14 h-14 rounded-full"
              />
              <div>
                <div className="font-semibold">
                  {sel?.contact?.name || "Contato"}
                </div>
