...
describe('reminder provider fallback', () => {
  let makeApp;
  let authHeaderFor;
  let resetDb;
  let app;
  ...
  it('retorna 424 quando whatsapp não configurado', async () => {
    process.env.WHATSAPP_PROVIDER = 'none';
    sendWhatsAppMock.mockImplementationOnce(() => {
      throw new ProviderNotConfiguredMock('WHATSAPP_NOT_CONFIGURED');
    });
    const h = authHeaderFor({ role: 'orgAdmin' });
    const r = await request(app)
      .post('/api/calendar/events/evt-3/remind')
      .set(h)
      .send({ to: '+554199999999', channel: 'whatsapp', text: 'oi' });
