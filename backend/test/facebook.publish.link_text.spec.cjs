...
const mockQuery = jest.fn((sql, params) => {
  ...
  if (sql.startsWith('INSERT INTO facebook_publish_jobs')) {
    const job = { id:'job'+(listJobs.length+1), status:'creating' };
    listJobs.push(job);
    return { rows:[{ id: job.id, status:'creating' }] };
  }
...
test('publish text', async () => {
  global.fetch = jest.fn().mockResolvedValue({ status:200, json:()=>Promise.resolve({ id:'t1' }) });
  const res = await request(app()).post('/api/orgs/o1/facebook/pages/pg1/publish').set('X-Org-Id','o1').send({ type:'text', message:'hello' });
  expect(res.statusCode).toBe(200);
  expect(res.body.published_post_id).toBe('t1');
});

test('publish link', async () => {
  global.fetch = jest.fn().mockResolvedValue({ status:200, json:()=>Promise.resolve({ id:'l1' }) });
  const res = await request(app()).post('/api/orgs/o1/facebook/pages/pg1/publish').set('X-Org-Id','o1').send({ type:'link', link:'http://x', message:'hey' });
  expect(res.statusCode).toBe(200);
  expect(res.body.published_post_id).toBe('l1');
});
