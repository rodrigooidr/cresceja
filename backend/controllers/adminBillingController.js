...
export async function setPlan(req, res, next) {
  try {
    ...
    const prov = getProvider(provider);
      ...
      await rootQuery('UPDATE subscriptions SET provider_subscription_id=$1 WHERE id=$2', [external.id, subId]);
    ...
    await auditLog(req.db, { user_email: req.user.email, action: 'set_plan', entity: 'subscription', entity_id: subId, payload: { orgId, plan_id } });
