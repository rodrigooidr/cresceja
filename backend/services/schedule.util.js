...
function weightedSampleWithoutReplacement(arr, k, rnd) {
...
function holidayBoost(dateStr, holidays) {
    if (!holidays.has(dateStr))
        return 1.0;
    return 1.2; // can tune per sector later
}
function pickTimeInWindows(date, windows, rnd) {
    if (!windows.length)
        return null;
...
function withinGap(target, slots, minGapH) {
...
/* ---------------------------- Core generation ---------------------------- */
export function generateSchedule(args, ctx) {
    ...
    const baseDates = weightedSampleWithoutReplacement(weighted, Math.min(args.frequency, weighted.length), rnd);
    ...
    // helpers
    const maxPer = { ig: ctx.plan.igDaily, fb: ctx.plan.fbDaily };
    ...
    for (const baseDate of pickedDates) {
        ...
        const targets = JSON.parse(JSON.stringify(args.defaultTargets || {
...
/* ------------------------------ Placements ------------------------------- */
function tryPlaceOnDate(date, targets, tz, windows, dayMap, maxPer, minGapH, rnd) {
    ...
    for (let attempt = 0; attempt < 6; attempt++) {
        ...
        if (igOk && fbOk) {
            if (targets.ig?.enabled) {
                load.igCount++;
                load.igSlots.push(at);
            }
            if (targets.fb?.enabled) {
                load.fbCount++;
                load.fbSlots.push(at);
            }
            return {
                date: dateStr,
...
function tryAdjacentDays(base, radiusDays, targets, tz, windows, dayMap, maxPer, minGapH, rnd) {
    for (let off = 1; off <= radiusDays; off++) {
        for (const sign of [+1, -1]) {
            const d = base.plus({ days: sign * off });
            if (!isSameMonth(base, d))
                continue; // keep inside month
