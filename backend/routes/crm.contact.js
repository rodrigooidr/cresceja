...
// POST /crm/contact  — idempotente (upsert por org_id + phone_e164)
router.post("/crm/contact", async (req, res, next) => {
  try {
    ...
    const email = payload.email
      ...
      : null;
    const birthdate = payload.birthdate ?? null; // 'YYYY-MM-DD'
    const phone_e164 = String(payload.phone_e164 ?? "").trim();

    if (!name || !email || !birthdate || !phone_e164) {
      return res.status(400).json({
        error: "validation",
        details: "name, email, birthdate, phone_e164 are required",
      });
    }

    // UPSERT por (org_id, phone_e164)
    const upsertSql = `
    ...
    // Garante o identity WhatsApp apontando pro contato
    await query(
   ...
   ON CONFLICT (org_id, channel, identity) DO UPDATE
     SET contact_id = EXCLUDED.contact_id`,
      [orgId, phone_e164, contact.id]
    );

    return res.json({ ok: true, contact });
  } catch (err) {
    // Se por acaso cair num 23505 (race), busca e retorna o existente
    if (err?.code === "23505") {
      try {
        const exist = await query(
          `SELECT * FROM public.contacts WHERE org_id = $1 AND phone_e164 = $2 LIMIT 1`,
          [req.user?.org_id || req.get("X-Org-Id"), req.body?.phone_e164]
        );
        if (exist.rows[0]) {
          return res.json({ ok: true, contact: exist.rows[0] });
        }
      } catch (_) {}
    }
    next(err);
