...
// … seu GET by id fica como já está …
...
const UpdateSchema = z.object({
  ...
  status: z.enum(['active','inactive']).optional(), // mapeie 'Ativa' no front para 'active'
...
router.put('/:orgId', async (req, res, next) => {
  try {
    ...
    // status amigável vindo do front (ex.: "Ativa") -> 'active'
    const statusNorm = (body.status || '').toString().toLowerCase();
    const status =
      statusNorm === 'ativa' ? 'active' :
      statusNorm === 'inativa' ? 'inactive' :
      (['active','inactive'].includes(statusNorm) ? statusNorm : undefined);
